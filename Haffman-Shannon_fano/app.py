...
class CompressionApp(tk.Tk):
    def __init__(self):
        ...
        self.action_button = tk.Button(
            button_frame, text="Сжать текст", command=self.compress_text)
        self.action_button.pack(side=tk.LEFT, padx=5)
        tk.Button(button_frame, text="Сброс", command=self.reset_text).pack(
            side=tk.LEFT, padx=5)

        # Область для вывода закодированного текста
        tk.Label(self, text="Закодированный текст:").pack(pady=5)
        self.encoded_output = tk.Text(self, height=5, state=tk.DISABLED)
    ...
    def update_mode(self):
        ...
        if mode == "encode":
            ...
            self.action_button.config(
                text="Сжать текст", command=self.compress_text)
        else:
            ...
            self.action_button.config(
                text="Дешифровать текст", command=self.decompress_text)

    def compress_text(self):
        text = self.text_input.get("1.0", tk.END).strip()
        if not text:
            messagebox.showwarning(
                "Предупреждение", "Пожалуйста, введите текст для сжатия.")
            return
        ...
        try:
            bit_sequence = ''.join(codes[char] for char in text)
        except KeyError:
            messagebox.showerror(
                "Ошибка", "Не удалось закодировать текст. Проверьте входные данные.")
            return
    ...
    def decompress_text(self):
        ...
        if not encoded_text or not mapping_str:
            messagebox.showwarning(
                "Предупреждение", "Введите закодированный текст и список кодов.")
            return
        ...
        if current_code:
            messagebox.showwarning(
                "Предупреждение", "Некорректный код: остаток не может быть декодирован.")
            return
